...
QUnit.module('atam-cli', function(hooks) {
  ...
  QUnit.module('adapter', function() {
    ...
    QUnit.test('should move an adapter', async function(assert) {
      let result = await execa(EXECUTABLE_PATH, ['adapter', 'sample', dest], execOpts);
    ...
    QUnit.test('should move an adapter from a namespace', async function(assert) {
      let result = await execa(EXECUTABLE_PATH, ['adapter', 'sample', dest, '-f','my-adapters'], execOpts);
    ...
    QUnit.test('should not move an adapter with dry-run', async function(assert) {
      let result = await execa(EXECUTABLE_PATH, ['adapter', 'sample', dest, '-d'], execOpts);
    ...
    QUnit.test('should move a constant', async function(assert) {
      const dest = 'packages/engines/dashboards';
      let result = await execa(EXECUTABLE_PATH, ['constant', 'sample', dest], execOpts);
