...
QUnit.module('atam-cli', function (hooks) {
  QUnit.module('component', function () {
    ...
    hooks.afterEach(async function () {
      ...
      await fs.remove(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample2-test.js'))
      ...
      await fs.remove(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample3-test.js'))
    })
    ...
    QUnit.test('should move a component', async function (assert) {
      ...
      assert.ok(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample3-test.js')))
    })

    QUnit.test('should move a component without pods', async function (assert) {
      const result = await execa(EXECUTABLE_PATH, ['component', 'sample', dest, '--pods', 'false'], execOpts)
      ...
      assert.ok(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample-test.js')))
    })

    QUnit.test('should move a component from a namespace', async function (assert) {
      const result = await execa(EXECUTABLE_PATH, ['component', 'sample', dest, '-f', 'my-components', '--pods', 'false'], execOpts)
    ...
    QUnit.test('should not move a component with dry-run', async function (assert) {
      const result = await execa(EXECUTABLE_PATH, ['component', 'sample', dest, '-d', '--pods', 'false'], execOpts)
      ...
      assert.notOk(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample-test.js')))
    })

    QUnit.test('should emit a warning if there is no test', async function (assert) {
      const result = await execa(EXECUTABLE_PATH, ['component', 'sample2', dest, '--pods', 'false'], execOpts)
      ...
      assert.notOk(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/integration/components/sample2-test.js')))
