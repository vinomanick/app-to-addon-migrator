...
QUnit.module('atam-cli', function (hooks) {
  QUnit.module('helper', function () {
    ...
    QUnit.test('should move an helper', async function (assert) {
      ...
      assert.ok(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/unit/helpers/sample-test.js')))
    })

    QUnit.test('should move an helper from a namespace', async function (assert) {
      const result = await execa(EXECUTABLE_PATH, ['helper', 'sample', dest, '-f', 'my-helpers'], execOpts)
    ...
    QUnit.test('should not move an helper with dry-run', async function (assert) {
      ...
      assert.notOk(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/unit/helpers/sample-test.js')))
    })
    ...
    QUnit.test('should emit a warning if there is no test', async function (assert) {
      ...
      assert.notOk(fs.pathExistsSync(path.join(FIXTURE_PATH, dest, 'tests/unit/helpers/sample2-test.js')))
